...
function WorldMapTerrainLayerInner({
  ...
  }, [enabled, visibleChunks, containerWidth, containerHeight, view, dim, chunkScope]);
  ...
  useEffect(() => {
    ...
    const pump = () => {
      while (inflightRef.current < MAX_CONCURRENT && pendingRef.current.length > 0) {
        ...
        const url = buildWorldMapTileUrl(dim, next.chunkX, next.chunkZ, token, networkId, view);
