# AI Assistant Technical Notes (Handoff)

本文是给后续 agent 继续开发 `AdvanceDataMonitor429` 中 AI assistant 功能的当前状态交接文档。内容基于当前代码实现整理；旧的“规则优先”描述已经不适用：现在主路径是 AI 结构化意图抽取，规则 parser 只作为 fallback 和少量本地上下文修复使用。
...
- AE2 查询半径、候选上限等参数仍硬编码，例如附近 32 格、候选上限 10、batch lines 上限 8。
...
- 配置化硬编码参数：AE2 查询半径、候选数量、batch line 上限、local inventory fallback 上限等。

## 12. 给后续 agent 的快速任务指南
