...
public class QqBotCoreTest {
    ...
    public void intentExplicitPrefixesAndKeywords() {
        ...
        QqBotIntentClassifier.Decision link = QqBotIntentClassifier.classify(
            cfg,
            "https://textech.top/tps");
        assertEquals(QqBotIntentClassifier.Owner.ASTRBOT, link.owner);
        assertEquals("default_astrbot", link.reason);

        QqBotIntentClassifier.Decision proseAndLink = QqBotIntentClassifier.classify(
            cfg,
            "请看 https://example.com/textech，然后查下仪表盘告警");
