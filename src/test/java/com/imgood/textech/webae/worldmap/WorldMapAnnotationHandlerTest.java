...
public class WorldMapAnnotationHandlerTest {
    ...
    public void setup() throws IOException {
        tempDirectory = Files.createTempDirectory("worldmap-annotation-handler-test")
            .toFile();
    ...
    public void crudUsesExpectedStatusesEnvelopesAndAuthenticatedActor() throws Exception {
        ...
        NanoHTTPD.Response listed = WorldMapAnnotationHandler.handleList(listParams, OWNER, ownerSession(), service);
        ...
        assertEquals(
            "Updated",
            updatedAnnotation.get("label")
                .getAsString());
        assertEquals(
            "Changed note",
            updatedAnnotation.get("note")
                .getAsString());
        assertEquals(
            ACTOR,
            updatedAnnotation.get("createdBy")
                .getAsString());

        NanoHTTPD.Response deleted = WorldMapAnnotationHandler
            .handleDelete(id, params("network", String.valueOf(NETWORK)), OWNER, ownerSession(), service);
        assertEquals(NanoHTTPD.Response.Status.OK, deleted.getStatus());
        JsonObject deletedJson = json(deleted);
        assertTrue(
            deletedJson.get("success")
                .getAsBoolean());
        assertEquals(
            id,
            deletedJson.getAsJsonObject("annotation")
                .get("id")
                .getAsString());

        NanoHTTPD.Response empty = WorldMapAnnotationHandler.handleList(listParams, OWNER, ownerSession(), service);
        assertEquals(
            0,
            json(empty).getAsJsonArray("annotations")
                .size());
    }
    ...
    public void createAndUpdateRequireObjectRootAndEveryExplicitPrimitiveField() {
        assertStatus(
            ...
            WorldMapAnnotationHandler
                .handleCreate(Collections.<String, String>emptyMap(), "[]", OWNER, ownerSession(), service));
        assertStatus(
            NanoHTTPD.Response.Status.BAD_REQUEST,
            WorldMapAnnotationHandler
                .handleCreate(Collections.<String, String>emptyMap(), "{", OWNER, ownerSession(), service));
        ...
        for (String field : REQUIRED_FIELDS) {
            ...
            body.remove(field);
            NanoHTTPD.Response response = WorldMapAnnotationHandler
                .handleCreate(Collections.<String, String>emptyMap(), body.toString(), OWNER, ownerSession(), service);
        ...
        assertStatus(
            NanoHTTPD.Response.Status.BAD_REQUEST,
            WorldMapAnnotationHandler.handleCreate(
                Collections.<String, String>emptyMap(),
                wrongType.toString(),
                OWNER,
                ownerSession(),
        ...
        assertStatus(
            NanoHTTPD.Response.Status.BAD_REQUEST,
            WorldMapAnnotationHandler.handleUpdate(
                UUID.randomUUID()
                    .toString(),
                Collections.<String, String>emptyMap(),
    ...
    public void mutationsRejectQueryBodyMismatchInvalidUuidAndGuestWrites() {
        ...
        assertStatus(
            NanoHTTPD.Response.Status.BAD_REQUEST,
            WorldMapAnnotationHandler.handleCreate(
                params("network", String.valueOf(NETWORK + 1)),
                body.toString(),
                OWNER,
                ownerSession(),
                service));
        ...
        assertStatus(
            NanoHTTPD.Response.Status.BAD_REQUEST,
            WorldMapAnnotationHandler.handleDelete(
                "not-a-uuid",
                params("network", String.valueOf(NETWORK)),
                OWNER,
                ownerSession(),
        ...
        WebAuthSession guest = new WebAuthSession(
            ...
            "guest-actor",
            "Guest");
        NanoHTTPD.Response guestResponse = WorldMapAnnotationHandler
            .handleCreate(Collections.<String, String>emptyMap(), body.toString(), OWNER, guest, service);
        assertEquals(NanoHTTPD.Response.Status.FORBIDDEN, guestResponse.getStatus());
        try {
            assertEquals(
                "guest_readonly",
                json(guestResponse).get("code")
                    .getAsString());
        } catch (IOException e) {
    ...
    public void missingAnnotationMapsToNotFound() throws Exception {
        ...
        NanoHTTPD.Response update = WorldMapAnnotationHandler
            .handleUpdate(id, Collections.<String, String>emptyMap(), body.toString(), OWNER, ownerSession(), service);
