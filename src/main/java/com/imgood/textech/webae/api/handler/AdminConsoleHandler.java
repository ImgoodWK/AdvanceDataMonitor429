    ...
    public static NanoHTTPD.Response handle(String uri, NanoHTTPD.Method method, String body, WebAuthSession auth,
        ...
        try {
            ...
            if ("/api/admin/server-console/execute".equals(uri) && method == NanoHTTPD.Method.POST) {
                ...
                Submission submission = AdminCommandService
                    .submit(request.command, request.confirmed, auth.actorUuid, auth.actorName);
            ...
            if ("/api/admin/server-console/presets".equals(uri) && method == NanoHTTPD.Method.PUT) {
                ...
                CommandPreset preset = AdminConsoleStore.instance()
                    .savePreset(request.id, request.label, request.command, request.description, auth.actorName);
                JsonObject response = success();
