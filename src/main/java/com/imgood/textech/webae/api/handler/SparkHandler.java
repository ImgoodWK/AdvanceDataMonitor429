...
public final class SparkHandler {
    ...
    public static NanoHTTPD.Response handle(String uri, NanoHTTPD.Method method, String body, WebAuthSession auth,
        ...
        if (!WebAuthAdminCheck.isAdmin(auth, adminHeader)) {
            return error(
                ...
                "admin_required",
                "Admin permission is required for Spark profiler control.");
        }
        if (method == NanoHTTPD.Method.POST && "/api/spark/analyze".equals(uri)) {
            try {
                JsonObject request = new JsonParser().parse(body == null ? "{}" : body)
                    .getAsJsonObject();
                JsonArray idsJson = request.getAsJsonArray("profileIds");
                List<String> ids = new java.util.ArrayList<String>();
                if (idsJson != null) {
                    for (int i = 0; i < idsJson.size(); i++) ids.add(
                        idsJson.get(i)
                            .getAsString());
                }
                String locale = request.has("locale") ? request.get("locale")
                    .getAsString() : "zh_CN";
                String aiSource = request.has("aiSource") ? request.get("aiSource")
                    .getAsString() : "";
                boolean browser;
                try {
                ...
                if (browser) {
                    return json(
                        ...
                        "{\"success\":true,\"request\":" + GSON.toJson(SparkAiAnalysisService.prepare(ids, locale))
                            + "}");
                }
        ...
        if (method == NanoHTTPD.Method.POST && "/api/spark/profile".equals(uri)) {
            ...
            try {
                if (body != null && !body.trim()
                    .isEmpty()) {
                    JsonObject request = new JsonParser().parse(body)
                        .getAsJsonObject();
                    if (request.has("durationSeconds")) duration = request.get("durationSeconds")
                        .getAsInt();
                    if (request.has("mode")) mode = request.get("mode")
                        .getAsString();
                    if (request.has("intervalMillis")) intervalMillis = request.get("intervalMillis")
                        .getAsInt();
                    if (request.has("onlyTicksOverMillis")) {
                        onlyTicksOverMillis = request.get("onlyTicksOverMillis")
                            .getAsInt();
                    }
                }
            } catch (Exception e) {
            ...
            try {
                SparkProfile profile = SparkService
                    .start(duration, auth.actorName, mode, intervalMillis, onlyTicksOverMillis);
                return json(
                    NanoHTTPD.Response.Status.OK,
        ...
        if (method == NanoHTTPD.Method.DELETE && uri.startsWith("/api/spark/history/")) {
            ...
            if (profile != null && profile.isActive()) {
                return error(
                    NanoHTTPD.Response.Status.CONFLICT,
                    "spark_active",
                    "Stop the active Spark run before deleting it.");
            }
            boolean removed = SparkProfileStore.instance()
                .remove(id);
        ...
        return error(
            ...
            "method_not_allowed",
            "Unsupported Spark endpoint or method.");
    }
    ...
    private static NanoHTTPD.Response status() {
        ...
        return json(
            NanoHTTPD.Response.Status.OK,
            "{\"success\":true,\"enabled\":true,\"available\":true,\"adminOnly\":true" + ",\"defaultDurationSeconds\":"
                ...
                + (current != null)
                + ",\"current\":"
                + (current == null ? "null" : summary(current).toString())
                + ",\"history\":"
                + history.toString()
                + "}");
    }
    ...
    private static JsonObject summary(SparkProfile profile) {
        JsonObject value = GSON.toJsonTree(profile)
            .getAsJsonObject();
        value.addProperty("messageCount", profile.messages == null ? 0 : profile.messages.size());
        value.addProperty(
            "baselineMessageCount",
            profile.baselineMessages == null ? 0 : profile.baselineMessages.size());
        value.addProperty(
            "completionMessageCount",
            profile.completionMessages == null ? 0 : profile.completionMessages.size());
    ...
    private static NanoHTTPD.Response error(NanoHTTPD.Response.Status status, String code, String message) {
        return json(
            status,
            "{\"success\":false,\"code\":\"" + escape(code) + "\",\"message\":\"" + escape(message) + "\"}");
    }
