...
import com.imgood.textech.webae.worldmap.WorldMapDirectCaptureBridge;
...
/**
 * C→S: client GL capture response for SP direct tile serve. Packet ID 46.
 */
public class PacketWorldMapDirectCaptureResponse implements IMessage {
    ...
    public static final int MAX_PNG_BYTES = 1024 * 1024;
    ...
    public PacketWorldMapDirectCaptureResponse() {}
    ...
    public void toBytes(ByteBuf buf) {
        ...
        if (totalChunks < 1 || totalChunks > MAX_TOTAL_CHUNKS || chunkIndex < 0 || chunkIndex >= totalChunks
            ...
            throw new IllegalArgumentException("Invalid world map capture response chunk");
        }
        buf.writeInt(chunkIndex);
        buf.writeInt(totalChunks);
        if (png != null && png.length > 0) {
            if (png.length > MAX_CHUNK_BYTES || !success) {
                throw new IllegalArgumentException("World map capture response exceeds packet limit");
            }
    ...
    public void fromBytes(ByteBuf buf) {
        ...
        try {
            ...
            if (success && png.length == 0) {
                throw new IllegalArgumentException("Empty successful world map capture response");
            }
            if (!success && png.length != 0) {
                throw new IllegalArgumentException("Failed world map capture response contains data");
            }
            if (buf.isReadable()) {
                throw new IllegalArgumentException("World map capture response has trailing bytes");
            }
    ...
    /** Client-side sender for a bounded, ordered world-map capture response. */
    ...
    public static class Handler implements IMessageHandler<PacketWorldMapDirectCaptureResponse, IMessage> {

        @Override
        public IMessage onMessage(final PacketWorldMapDirectCaptureResponse message, MessageContext ctx) {
