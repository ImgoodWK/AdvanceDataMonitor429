...
public final class WorldMapDynmapTileRoot {
    ...
    public static Path getTileRoot() {
        ...
        if (configured != null && !configured.trim().isEmpty()) {
            ...
            if (relativeDir.isDirectory()) {
                ...
                AdvanceDataMonitor.LOG.info("[WebAE-WorldMap] Dynmap tile root from config (relative): {}",
                    cachedTileRoot.toAbsolutePath());
                return cachedTileRoot;
            }
            AdvanceDataMonitor.LOG.warn(
                "[WebAE-WorldMap] Configured worldMapDynmapTileRoot '{}' is not a valid directory", configured);
        }
        // Fallback: dynmap/web/tiles/
        File defaultDir = new File(DEFAULT_RELATIVE);
        if (defaultDir.isDirectory()) {
            cachedTileRoot = defaultDir.toPath();
            AdvanceDataMonitor.LOG.info("[WebAE-WorldMap] Dynmap tile root from default: {}",
                cachedTileRoot.toAbsolutePath());
            return cachedTileRoot;
        }
    ...
    public static Path resolveWorldTiles(String worldName) {
        ...
        if (root == null || worldName == null || worldName.trim().isEmpty()) {
            return null;
        }
