...
public class AssistantAiIntentService {
    ...
    private String buildSystemPrompt(String locale) {
        ...
        sb.append(
            zh ? "CHAT仅用于不涉及查询AE2、合成、取出、确认、取消或管理计划的普通对话。每个请求的动作一个任务，最�?个任务�?
        ...
        sb.append(
            zh ? "QUERY_BYTES表示查询AE2存储网络的字节占用详细信息（物品/流体各占�?总量字节、百分比）和是否有无限存储元件。target留空�?
        ...
        sb.append("\n")
            .append(
                zh ? "如果用户只说�?�?给而没有明确的合成vs存储上下文，返回CLARIFY而不是猜测�?
        ...
        sb.append(zh ? "用户语言�? + effectiveLocale + "�? : "The user's locale is " + effectiveLocale + ".");
        return sb.toString();
    }
}
