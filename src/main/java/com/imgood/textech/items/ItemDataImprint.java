...
public class ItemDataImprint extends Item {
    ...
    public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player) {
        if (player.isSneaking()) {
            if (!world.isRemote) {
                clearNBTData(stack);
                player.addChatMessage(new ChatComponentText("§a已清除所有映录数�?"));
            }
        } else {
            if (world.isRemote) {
    ...
    private void saveBlockData(ItemStack stack, World world, int x, int y, int z, EntityPlayer player) {
        ...
        if (te instanceof TileEntityAdvanceDataMonitor) {
            if (nbt != null && nbt.hasKey("boundPos")) {
                ...
                if (duplicate) {
                    player.addChatMessage(new ChatComponentText("§c该坐�?(" + newCoordStr + ") 已存在于数据显示器的数据中，跳过添加�?));
                    return;
                }
                ...
                if (!found) {
                    player.addChatMessage(new ChatComponentText("§c数据显示器数据已满（最�?00条），无法添加新数据�?));
                    return;
                }
                ...
                player.addChatMessage(
                    new ChatComponentText("§a已将坐标 (" + newCoordStr + ") 添加到高级数据显示器（索�?" + nextIndex + "�?));
                return;
            } else {
                player.addChatMessage(
                    new ChatComponentText("§c数据映录器尚未映录任何坐标。请�?shift+右键 一个方块映录其数据，再 shift+右键 高级数据显示器来添加�?));
                return;
            }
        }
        ...
        if (te != null) {
            ...
            player.addChatMessage(new ChatComponentText("§a已成功映录方块数�?"));
    ...
    private void openNbtGui(ItemStack stack, EntityPlayer player) {
        if (!stack.hasTagCompound()) {
            player.addChatMessage(new ChatComponentText("§c物品未存储任何数�?"));
            return;
        }
        ...
        if (nbt.hasKey("boundTE")) {
            ...
            player.addChatMessage(new ChatComponentText("§c映录的方块没�?NBT 数据!"));
        }
    }
