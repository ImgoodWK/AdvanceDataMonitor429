...
public final class ConfigDescriptions {
    ...
    static {
        ...
        put(
            "webConsole",
            "aiKeyMode",
            "Deprecated mutual WebAE AI key mode (server/browser). Prefer aiServerKeyEnabled and aiBrowserKeyEnabled. Still used once to seed the new flags when those keys are first written.",
            "已弃用的互斥 WebAE AI Key 模式（server/browser）。请改用 aiServerKeyEnabled 与 aiBrowserKeyEnabled；首次写入新键时仍会用本项做一次迁移种子。");
        put(
            "webConsole",
            "aiServerKeyEnabled",
            "Allow WebAE to use admin-managed shared AI profiles (encrypted server-side keys). Can be enabled together with aiBrowserKeyEnabled. Default true (false when migrating from legacy aiKeyMode=browser).",
            "是否允许 WebAE 使用管理员维护的共享 AI 配置（服务端加密 Key）。可与 aiBrowserKeyEnabled 同时开启。默认 true（从旧 aiKeyMode=browser 迁移时为 false）。");
        put(
            "webConsole",
            "aiBrowserKeyEnabled",
            "Allow each browser to store its own AI profiles in localStorage and call providers directly. Keys are never sent to WebAE. Can be enabled together with aiServerKeyEnabled. Default false (true when migrating from legacy aiKeyMode=browser).",
            "是否允许每个浏览器在 localStorage 保存自己的 AI 配置并直连 Provider；Key 不会发送到 WebAE。可与 aiServerKeyEnabled 同时开启。默认 false（从旧 aiKeyMode=browser 迁移时为 true）。");
        put(
            "webConsole",
            "sparkEnabled",
            "Enable the WebAE Spark profiler tab in the admin console and API when the optional Spark mod is installed.",
            "安装可选 Spark 模组时，是否启用管理控制台中的 WebAE Spark 性能分析标签和接口。");
        put(
            "webConsole",
            "sparkMaxHistory",
        ...
        put(
            ...
            "Default Spark profiler duration requested from the WebAE admin console, in seconds.",
            "WebAE 管理控制台发起 Spark 分析时的默认时长（秒）。");
        put(
            "webConsole",
