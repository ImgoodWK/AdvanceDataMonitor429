...
public class TickHandler {
                ...
                if (now - lastOutput > 1000) {
                    ...
                    if (nbt != null && nbt.hasKey("Position")) {
                        BlockPos pos = new BlockPos(
                            nbt.getCompoundTag("Position").getInteger("x"),
                            nbt.getCompoundTag("Position").getInteger("y"),
                            nbt.getCompoundTag("Position").getInteger("z")
                        );
                    }
                    /*
    ...
    private String getNbtValue(NBTTagCompound nbt, String path) {
        ...
        return current.getTag(parts[parts.length - 1]).toString();
    }
}
