...
public class DataWeave extends Item {
    public DataWeave() {
        setMaxStackSize(1);
        ...
        if (player.isSneaking()) {
            ...
            if (world.isRemote) {
                ...
                if (te != null) {
                    ...
                    te.writeToNBT(teNbt);
                    String jsonData = NBTJsonParser.parseNBTToJson(teNbt).toString();
    ...
    private void openNbtGui(ItemStack stack, EntityPlayer player) {
        if (stack.hasTagCompound()) {
            ...
            if (nbt.hasKey("boundPos")) {
                ...
                JsonObject json = NBTJsonParser.parseNBTToJson(tileNBT);
                System.out.println("json"+json);
                Minecraft.getMinecraft().displayGuiScreen(new NBTViewerGUI(json));
            } else {
                // 提示未绑定数据
