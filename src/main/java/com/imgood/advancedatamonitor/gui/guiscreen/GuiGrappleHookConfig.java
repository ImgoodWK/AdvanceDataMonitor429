...
public class GuiGrappleHookConfig extends ADM_GuiScreen {
    ...
    public void initGui() {
        ...
        // 重命名输入框初始位置（renamingActive 时动态更新 xPosition/yPosition）
    ...
    private void previewSelectedRoute() {
        ...
        // 不关闭 GUI：用户可在 GUI 中继续切换其他路线的预览状态
    }
    ...
    private void ensureRenameRowVisible() {
        ...
        // 重命名输入框出现在选中行下一行位置，需要选中行 + 下一行都可见
    ...
    private void updateRenameFieldPosition() {
        ...
        // ADM_GuiTextField 内部存 xPosition=x-21 / yPosition=y-8（构造时偏移），这里直接重设以动态定位。
