...
- `src/main/java/com/imgood/textech/assistant/AssistantServerServices.java`
  ...
  - 查找玩家附近默认 32 格（可通过 `assistant.linkSearchRadius` 配置，范围 4–128）内 `TileEntityAdvanceCraftingLink` / `TileEntityAdvanceStorageLink`。
...
- `WITHDRAW_ITEM` 单任务：调用 `requestWithdrawCandidates()`，服务端通过 `AssistantServerServices.withdrawCandidates()` 搜索 AE2 存储网络（依赖附近默认 32 格（可通过 `assistant.linkSearchRadius` 配置，范围 4–128）内的 `TileEntityAdvanceStorageLink`）。候选项返回后通过 `confirmOption()` 走 `submitWithdraw()`。
