...
- 无限存储元件检测通过 `AeCompat.cells().readItemCellStats()` / `readFluidCellStats()`（Legacy 路径含 GlodBlock `FluidCellInventoryHandler` 反射）；类名关键词与字节阈值逻辑见 `compat/ae/legacy/LegacyAeCellStatsAdapter`。GTNH 2.9.0+ 使用 Native 路径时见 `docs/zh/developer/ae-compat-290.md`。
