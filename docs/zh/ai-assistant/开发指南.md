...
> 受众：开发者 / Cursor Agent · 最后同步：2026-06<br>
...
- `src/main/java/com/imgood/textech/assistant/AssistantCandidateDelivery.java`
  - `assistantQueryCandidateBatchSize` 是每批条数上限；服务端还会按实际编码大小预检，在 28 KiB 目标内递归细分并连续 `sendTo` 多个 `CANDIDATES` 包。所有分包构造成功后才发送；单个候选自身超过安全 wire budget 时返回本地化错误，不会静默丢项或发送半套列表。超 `assistantMaxQueryCandidates` 时仍附带截断提示。
  - `PacketAssistantAction`/`PacketAssistantResponse` 使用 30000 bytes 包体预算；每个压缩 NBT 字段最多 24 KiB，response gzip 展开量最多 2 MiB，并拒绝 NBT/packet 尾随数据。动态分包使用显式 `rangeStart`/`rangeEnd` 保持 UI 范围准确。
...
- `src/main/java/com/imgood/textech/assistant/AssistantServerServices.java`
  ...
  - 新增 `bytesSummary()` — 查询字节占用/容量/百分比，并检测 AE2Things 无限存储元件。内部使用 `scanNetworkCellsForInfinite()` + `classifyCell()`（经 `compat/ae/AeCompat.cells()`）。
  - 查找玩家附近默认 32 格（可通过 `assistant.linkSearchRadius` 配置，范围 4–128）内的 `TileEntityAdvanceNetworkLink`（三合一链接器）。

### 格式化、辅助、存储和 lexicon

- `src/main/java/com/imgood/textech/assistant/AssistantFormatter.java`：候选项、批量订单行等面向聊天窗口的格式化输出。
- `src/main/java/com/imgood/textech/assistant/AssistantOperationsBriefing.java`：基地运维简报格式化器；统一状态标签、分段/总长度上限和敏感值脱敏。
...
- `src/main/java/com/imgood/textech/assistant/AssistantLexicon.java`：加载和提供 lexicon 数据；现在主要用于 fallback、时间/数量/词语清理、pending batch append 辅助解析。边缘标点清理接受文档约定的对称字符类，将其解析为 code point 集合并在线性时间内扫描两端；不支持的复杂表达式改用内置安全集合，不会把管理员正则直接作用于玩家文本。
...
- `src/main/java/com/imgood/textech/assistant/ai/WebSearchService.java`
  - 内置联网搜索层：多引擎适配（Tavily keyless、DuckDuckGo、Tavily、Brave、Serper、SearXNG）、结果归一化、自动降级、注入 user 消息。
...
- `src/main/java/com/imgood/textech/gui/guiscreen/GuiAISettings.java`
  - 游戏内 AI/voice 设置界面；面板超出 scaled viewport 时切换为三列字段、两行开关的紧凑布局。
...
- `webae/assistant/WebAssistantService.java` 复用 `AssistantAiIntentService.buildSystemPrompt()` 和 `AssistantAiIntentJsonParser`，因此 Web 页面与游戏内遵循同一个 intent/task schema；AI 不可用或输出非法时仍走 `AssistantIntentService` 词库回退。`[webConsole] aiKeyMode=server` 时由服务端生成计划，`browser` 时浏览器取得无密钥提示词、直连 Provider，再把计划交回同一解析器。
- WebAE 的查询、计划、合成候选、取出候选和传送点都调用同一组 `AssistantServerServices` / `TeleportService`。修改型动作不会由模型直接执行，而是生成绑定 Web actor/owner、5 分钟失效的随机确认令牌；候选 `ItemStack` 与 NBT 始终保留在服务端。
- `WebAiConfigStore` 是独立于游戏客户端 `ai-client-local.cfg` 的 WebAE 共享配置：共享模式由管理员写入 Key，AES-GCM 密文与独立主密钥位于 `TeXTech/WebAE/`，安全视图只有状态/末四位掩码。个人模式下 `runtime()` 硬禁用，浏览器 Key 只留在该站点 `localStorage` 并由 `webae-frontend/src/utils/personalAi.ts` 直连 Provider；任何 WebAE API 都不携带它。无论模式如何，访客工具只读，修改操作都需要服务端确认令牌。
- `WebAiHttpClient` 支持 OpenAI-compatible `/chat/completions`、Anthropic Messages 和 Gemini GenerateContent 三类协议；Base URL 强制 HTTPS，仅回环地址允许 HTTP。Provider 兼容性与端点变更应同步 WebAE 中英文开发者/用户文档。
- Spark AI 分析不是新意图：管理员从 Spark 页显式触发 `SparkAiAnalysisService`，只发送有界类别/线程/方法热点聚合；原始 Spark 输出、Viewer URL、身份和 API Key 不进入 prompt。
- QQ 群机器人 AI 也不是新的游戏内意图：`webae/qqbot/QqBotService` 复用 `WebAiCompletionService` 与可选共享搜索，仅提供自由对话和 TPS/人数/在线名单/内存/运行时间等本地只读命令。会话按 QQ 目标与用户隔离并限轮数/TTL/冷却，prompt 只注入有界服务器快照；QQ 侧不能触发合成、取出、传送或任意控制台命令。由于没有新增 `AssistantIntentType`，本变更不增加 `assistant-features.json` / `assistant-lexicon.json` 条目。
...
- `QUERY_BRIEFING` 同样通过 `requestServerQuery()` 发送标准 `PacketAssistantAction.query(...)`；服务端进入 `AssistantServerServices.query()` 的 `QUERY_BRIEFING` 分支。它严格只读，不执行下单、取出、传送或计划修改，也不增加新的 packet action/packet ID。
- `QUERY_BRIEFING` 追加在 `AssistantIntentType` 末尾，旧意图 ordinal 不变；它的 target 必须留空。
- `WITHDRAW_ITEM` 单任务：调用 `requestWithdrawCandidates()`，服务端通过 `AssistantServerServices.withdrawCandidates()` 搜索 AE2 存储网络（依赖附近默认 32 格（可通过 `assistant.linkSearchRadius` 配置，范围 4–128）内的 `TileEntityAdvanceNetworkLink`）。候选项返回后通过 `confirmOption()` 走 `submitWithdraw()`。
...
- `GuiAIChat` 用 `GuiResponsiveLayout` 将 600×450 首选面板约束进当前 scaled viewport；头部在窄于 370px 时改为两行，聊天区、滚动条、输入区和换行宽度共享运行时 bounds。
...
- `QUERY_BRIEFING` 由 `AssistantServerServices.operationsBriefing()` 组装七个独立分段：`network` → `networkSummary`、`networkHealth` → owner 隔离的 `NetworkHealthDiagnosticProvider` 缓存、`bytes` → `bytesSummary`、`power` → `WirelessPowerQuery`、`steam` → `WirelessSteamQuery`、`jobs` → `AssistantCraftJobManager.instance().summary`、`planner` → `PlannerServerService.briefingSummary`。常规来源由各自的 `briefingSection` 捕获异常；`networkHealth` 使用显式状态 section builder，并由独立的异常边界回退为 unavailable。因此任一来源失败都不会拖垮整份报告。
- 网络分段调用 `networkSummary(player, locale, false)`：可读已有记录，也可通过 `AssistantMonitorRegistry.findNearest()` 做只读附近搜索，但不会建立新的 monitor association。计划器分段只返回待办/完成/总数，不回显玩家自由文本。
- `networkHealth` 分段只格式化服务端 tick 已采样的 `NetworkHealthDiagnosticProvider.snapshotForOwner(playerUuid)`；不会再次扫描 World/TileEntity/AE Grid，也不会自行推导第二套健康结论。Provider 的 `healthy`、`degraded`、`failed`、`unknown` 状态会映射到简报展示状态，并在发包前限制 issue evidence 长度。
- `AssistantOperationsBriefing` 负责格式化 `healthy` / `attention` / `unavailable` 状态；单段同时受 `MAX_SECTION_CHARS=560` 与 480 UTF-8 bytes 限制，整份报告最多 4000 UTF-8 bytes，按 Unicode code point 边界截断，低于 `PacketAssistantResponse` 的 4096-byte message 字段。API key、token、secret、password 及 `sk-...` 样式敏感值（含 JSON/带引号形式）会脱敏。
- WebAE 复用同一 parser 与 `AssistantServerServices.query()` 通用分支，因此自动支持只读简报；本功能不增加 WebAE 页面、HTTP API 或确认令牌流程。
- 空 target 的 `QUERY_RECIPE` 会返回 recipe candidates，即 `AssistantSessionKind.RECIPE_CANDIDATES`，让用户后续确认某个候选查看详情。
- `AssistantServerServices` 查找附近默认 32 格（可通过 `assistant.linkSearchRadius` 配置，范围 4–128）：
  - 合成 / 存储 / 网络统计：统一 `TileEntityAdvanceNetworkLink`
- server service 当前覆盖：
...
- 历史独立测试入口 `src/test/java/test/AssistantIntentParserSuite.java` **已不在仓库中**；简报的聚焦 JUnit 回归位于 `src/test/java/com/imgood/textech/assistant/AssistantOperationsBriefingTest.java`。
- 建议/现有覆盖内容：
  ...
  - `QUERY_BRIEFING` JSON allow-list/schema 解析；本地 `briefingQueryWords` 触发；六段全成功及单段异常仍保留其他段；状态分类、单段/总长度上限与敏感值脱敏；标准 query 路径及无新增 packet ID。
- 当前工作区可用 `./gradlew.bat test --tests com.imgood.textech.assistant.AssistantOperationsBriefingTest --tests com.imgood.textech.network.packet.PacketAssistantResponseTest --no-daemon` 验证简报与响应包边界；未运行全量测试时不得表述为全量通过。
...
- AE2 查询半径、batch lines 上限等部分参数仍硬编码，例如 batch lines 上限 8；AE2 链接搜索半径已配置化（`assistant.linkSearchRadius`，默认 32，范围 4–128）。
- 批量提交按行数占用 AE2 计算槽；重复项如果未合并会占更多槽。
- 取出物品时背包空间不足会触发部分取出确认流程；规则 fallback 与 pending batch 确认均支持中英文关键词（如「确认」/ confirm / yes / ok）。
- 批量取出中某一行需要部分确认时，整个批量操作暂停，不能跳过该行继续后续行。
- 取出物品依赖 AE2 `extractItems` API，如果存储单元不支持提取（如部分特殊存储），会失败。
- 无限存储元件检测通过 `AeCompat.cells().readItemCellStats()` / `readFluidCellStats()`。TeXTech 2.0 始终绑定 NativeFluid 适配器；`compat/ae/legacy/LegacyAeCellStatsAdapter` 仅保留部分混合格式/迁移辅助，不代表旧整合包支持。详见 `docs/zh/developer/ae-compat-290.md`。
...
| 查询只读基地运维简报 | `QUERY_BRIEFING` | `AssistantIntentType`, `AssistantIntentService`, `AssistantAiIntentService`, `AssistantAiIntentJsonParser`, `AssistantController`, `AssistantServerServices.operationsBriefing()`, `AssistantOperationsBriefing` |
...
└─ TileEntityAdvanceNetworkLink.java ← 三合一高级网络链接器（网络统计 / 存储 / 合成）
```
