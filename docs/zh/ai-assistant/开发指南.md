...
- 历史独立测试入口 `src/test/java/test/AssistantIntentParserSuite.java` **已不在仓库中**；本地解析器验证请用 `.workspace/assistant-parser-suite/` 下的临时桩（不提交），或在依赖齐全后自行补回归测试。
- 建议覆盖内容（若恢复测试）：
...
- 后续 agent 如果修复依赖源或本地缓存后，应重新运行 `./gradlew.bat test`，再根据输出确认 parser 回归是否通过。
