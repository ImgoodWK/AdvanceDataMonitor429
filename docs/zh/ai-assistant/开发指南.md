...
- `src/main/java/com/imgood/textech/assistant/AssistantLexicon.java`：加载和提供 lexicon 数据；现在主要用于 fallback、时间/数量/词语清理、pending batch append 辅助解析。边缘标点清理会把旧的对称正则迁移为线性时间形式；超过 1024 个字符的输入改用 `trim()` 回退，避免管理员提供的正则形成无界工作量。
