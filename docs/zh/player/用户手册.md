...
**主要方块与物品**（游戏内显示名，详见 [开发者技术文档 §5](../developer/技术文档.md#5-方块物品与-tileentity)）：高级数据监视器、数据映录器、网络链接器、合成链接器、高级存储链接器、挂索节点、挂索器、高级存储链接元件、数据织尘/织形/涌流/织潮/织源元件、编织增幅卡、高级计划器、超能砂糖桔、至高天圣裁、高级数据监视器手册等。
...
右键打开内置 GUI 手册（章节导航 + 页面内容），涵盖方块物品说明、配置参考等。玩家**首次加入服务器**时自动赠送一本（`HandlerPlayerJoin`）。
