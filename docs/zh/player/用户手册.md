...
**主要方块与物品**（游戏内显示名，详见 [开发者技术文档 §5](../developer/技术文档.md#5-方块物品与-tileentity)）：高级数据监视器、数据映录器、网络链接器、合成链接器、高级存储链接器、挂索节点、挂索器、高级存储链接元件、数据织尘/织形/涌流/织潮/织源元件、编织增幅卡、高级计划器、超能砂糖桔、至高天圣裁、铽丝科技手册、次元口袋（含空间/翻页升级卡）等。
...
注册名：`textech:super_orange`（lang：`item.orange.name`）
...
配置分节 `[superOrange]`。首次加入游戏不自动赠送（与铽丝科技手册不同）。
