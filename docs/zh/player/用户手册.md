...
| 现象 | 说明 |
|------|------|
| 重启存档后数量变 1 | 重新加载世界后，口袋内已存物品的数量可能全部变成 1 个；与升级无关，属于存档读写问题。 |
| 悬浮窗角标显示 0 | 悬浮窗格子上的数量角标可能始终显示 0，与口袋内实际数量不符；右键打开的存储 GUI 内数量显示通常仍正常。 |

开发者排查细节见 [技术文档 §5.12](../developer/技术文档.md#512-次元口袋dimensional-pocket)。
