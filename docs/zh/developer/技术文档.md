...
AI 助手的服务端执行也依赖玩家附近的 Crafting Linker（默认搜索半径 32 格，可通过 `[assistant] linkSearchRadius` 配置，范围 4–128）：查询可合成候选、查询样板详情、提交合成、批量提交和取消服务端 job 都从这里进入 AE2 网络。取出物品操作则依赖附近的 Advanced Storage Linker，通过 `IStorageGrid` 搜索与提取物品。
...
- `LoaderRecipe.registerAssemblyLineRecipes()`：UHV 装配线，扫描高级存储链接器 90s @ UV，120s @ UHV；材料见 [用户手册 §3.14](../player/用户手册.md#314-物质球解压器)。
...
- `2`（`ADM_STORAGELINK_ID`）：Advanced Storage Linker GUI，服务端返回 `ContainerAdvanceStorageLink`，客户端返回 `GuiAdvanceStorageLink`。
...
所有 packet 共用 `TeXTech.ADMCHANEL`，注册在 `LoaderNetwork`（完整表见 `.cursor/rules/network-packets.mdc`）：

| ID | 包类 | 方向 | 用途 |
|----|------|------|------|
| 0 | `PacketItemNBT` | SERVER | 数据映录器 NBT 交互 |
| 1 | `PacketSynTileEntity` | CLIENT | 监视器 TE 下发客户端 |
| 2 | `PacketSynTileEntity` | SERVER | 客户端提交监视器配置 |
| 3 | — | — | 未使用 |
| 4 | `PacketRequestItemCountSync` | SERVER | 请求 Advanced Storage Linker 数量 |
| 5 | `PacketItemCountSync` | CLIENT | 返回 Advanced Storage Linker 数量 |
| 6 | `PacketAssistantAction` | SERVER | AI 助手操作 |
| 7 | `PacketAssistantResponse` | CLIENT | AI 助手响应 |
| 8–9 | `PacketPlannerSync` | SERVER / CLIENT | 计划器 NBT 上行/下发 |
| 10 | `PacketPlannerMerge` | SERVER | 计划器合并 |
| 11–12 | `PacketGrappleAction` / `PacketGrappleSync` | SERVER / CLIENT | 挂索动作/状态同步 |
| 13–14 | `PacketGrappleHookConfig` / `PacketGrappleAnchorConfig` | SERVER（14 双向） | 挂索器/节点配置 |
| 15 | `PacketMonitorRecord` | SERVER | 监视器坐标登记 |
| 16 | `PacketLinkScannerAction` | SERVER + CLIENT | 链接扫描器动作/同步 |
| 17–18 | `PacketGrapplePathAction` / `PacketGrapplePathSync` | SERVER / CLIENT | 挂索路径保存/下发 |
| 19–20 | `PacketPocketAction` | SERVER / CLIENT | 次元口袋 C→S 动作 / 响应占位 |
| 21 | `PacketPocketSync` | CLIENT | 次元口袋状态/物品栏下发 |
| 22 | `PacketSuperOrangeConfig` | SERVER | 超能砂糖桔设置 GUI 保存 |
| 23 | `PacketMatterBallDecompressorToggle` | SERVER | 物质球解压器输出模式/阻塞模式切换 |
