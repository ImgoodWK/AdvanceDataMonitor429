...
注意代码中存在少量历史命名拼写，例如 `RenderTeXTech`、`gui/custom/`。维护时优先判断是否已有外部引用或注册名依赖，避免仅为命名整洁破坏兼容性。
...
| 层级 | 类 | 职责 |
|------|-----|------|
| 状态 / 持久化 | `PocketState` / `PocketStore` / `PocketInventory` | 按玩家 UUID 维护槽位与升级；NBT 压缩写入 `<世界存档>/advancedatamonitor/pocket-<uuid>.dat` |
...
| 3 | **重启存档后口袋数据丢失**（JSON→NBT 存储迁移） | 旧版将口袋数据以 JSON 格式存储于 `config/textech/pocket-<uuid>.json`。新版改用 `CompressedStreamTools` 压缩 NBT 写入 `<世界存档>/advancedatamonitor/pocket-<uuid>.dat`，按世界存档隔离存储。 | `PocketStore` 重构为 `CompressedStreamTools` 读写；`铽丝科技` 新增 `WorldEvent.Load` 监听器设置世界存档目录。迁移期间若新路径无文件，自动降级使用旧 JSON 路径作为回退。 |
...
`GuiHandler` 当前定义十个 Forge GUI id：
...
主监视器 GUI 以 `GuiMainTeXTech` 为入口，子页面负责绑定目标、配置 AE2 Network Linker、Crafting Linker、Advanced Storage Linker、颜色和显示变换。AI 聊天与 AI 设置是独立 `GuiScreen`，由主界面按钮、命令或语音热键打开。
...
- `ai`：base URL、API key、model、network enabled、内置联网搜索（引擎/Key/SearXNG URL/结果数/降级）、streaming、timeout、max tokens、temperature、privacy flag、debug logging。客户端保存在 `ai-client-local.cfg`。
...
- 方块 / 物品外观：`RenderTeXTech`、`RenderAdvanceNetworkLink`、各 item renderer，由 `LoaderRender` 绑定。
