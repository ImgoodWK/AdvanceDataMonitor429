...
- 世界内监视器：`TileEntityTeXTech` 采集目标 TileEntity / AE2 链接器数据，客户端 TESR 渲染为图表、文本、合成状态或存储物品列表。
...
- `LoaderNetwork` 注册所有 packet，discriminator `0`–`21`（完整 ID 表见 `.cursor/rules/network-packets.mdc` 与 `LoaderNetwork.java`）。
...
`TileEntityTeXTech` 持有 `dataBoundList`，每个 index 对应一个 `NBTTagCompound` 显示项。显示项里保存目标坐标、字段名、采样间隔、颜色、缩放、旋转、数据类型等。服务端 `updateEntity()` 按每个显示项的 `interval` 采样：
...
`GuiHandler` 当前定义八个 Forge GUI id：
...
新增监视器显示类型：定义显示项 NBT schema，补 GUI 默认值，扩展 `TileEntityTeXTech.processTileEntityData()` 或相邻采集方法，实现 `IADMRender` 并在 `LoaderRender` 注册。
