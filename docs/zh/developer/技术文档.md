...
- `mixin/`：客户端 Mixin（NEI tooltip/布局、GUI 容器钩子），配置见 `mixins.advancedatamonitor.json`。
...
JSON 位于 `assets/advancedatamonitor/manual/`；`HandlerPlayerJoin` 首次加入赠送手册。

`GuiManual` 布局：左侧顶栏搜索框、可滚动章节列表（滚动条含顶/底跳转按钮）、右侧分页内容区。搜索对章节标题与全部页面正文建立索引（`ManualSearchIndex`），空格分隔 AND；拼音匹配内置 shadow **PinIn 1.6.0**，若加载 **NotEnoughCharacters** 则优先走其 `NecharUtils.contain`。详见 [用户手册 §3.12](../player/用户手册.md#312-高级数据监视器手册)。
...
| 层级 | 类 | 职责 |
|------|-----|------|
| 状态 / 持久化 | `PocketState` / `PocketStore` / `PocketInventory` | 按玩家 UUID 维护槽位与升级；JSON 写入 `config/advancedatamonitor/pocket-<uuid>.json` |
...
以下问题已在近期交互/悬浮窗改动后确认，**尚未修复**；排查时可优先对照上表类名。

| # | 现象 | 影响范围 | 初步排查方向 |
|---|------|----------|--------------|
| 1 | **重启存档（重新加载世界）后**，口袋内物品 `stackSize` 变为 **1** | 持久化数据；右键 GUI 与 JSON 存档均受影响 | `PocketStore` 读写 JSON 时 `stackSize` 字段是否丢失或被截断；`PocketState` / `PocketInventory` 加载路径是否与保存不一致 |
| 2 | **悬浮窗槽位角标始终显示 0**，与口袋实际数量无关 | 仅悬浮窗；右键存储 GUI 数量显示正常 | `PocketClientCache` 是否在 `PacketPocketSync` 后持有正确 `stackSize`；`GuiPocketOverlay` / `PocketStackOverlayRenderer` 渲染前 stack 是否为 null 或 count 为 0 |
...
所有 packet 共用 `AdvanceDataMonitor.ADMCHANEL`，注册在 `LoaderNetwork`（完整表见 `.cursor/rules/network-packets.mdc`）：
