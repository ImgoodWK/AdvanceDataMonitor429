...
- `webae/`：嵌入式 Web 控制台（NanoHTTPD HTTP 服务器 + REST API + 前端 SPA），子包 `auth/`、`api/handler/`、`cache/`、`dto/`、`power/`、`snapshot/`、`gt/`、`network/`、`recipe/`、`pattern/`、`icon/`、`player/`（玩家信息收集 + 皮肤 URL 解析）、`chat/`（聊天消息环形缓冲）；详细架构、API 端点表、网络包与子系统设计见 [WebAE 开发者手册](../webae/开发者手册.md)。
...
**完整 ID 表与新增接入步骤**：见 [`.cursor/rules/network-packets.mdc`](../../.cursor/rules/network-packets.mdc)（与源码保持同步，下一可用 ID = 50）。
...
| 22–23 | 超能砂糖桔 / 物质球解压器 |
| 24–25 | AI 助手菜单状态 |
| 26–36 | WebAE 配方/图标/Token/世界地图 HD 瓦片/图标 Nack |
