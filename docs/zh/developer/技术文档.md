...
主路径：WebAE「导出到游戏显示器」发布布局到 `TeXTech/WebAE/displays/` 并复制绑定 JSON。默认懒人体验：**无需 MCEF**——主机用系统 Chrome/Edge 优先无头截真实 `/embed/dashboard?...&capture=1`（`DisplayCaptureService`），空白则回退精简 `render.html`；客户端按距离拉 `frame.jpg` 贴监视器。可选开启 `webSurfaceUseMcef` 且安装 montoyo MCEF 时走本机 OSR。发布响应尽量带上浏览器 Origin 作为 `webaeOrigin`。MC Tick 不跑浏览器。旧快照路径仍可用。每台监视器最多 8 个网页绑定。

客户端 `WebSurfaceSourceRouter`：可选 MCEF → CDP 占位 → HttpFrame → Snapshot。
