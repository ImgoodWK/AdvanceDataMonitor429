...
- [5. 方块、物品与 TileEntity](#5-方块物品与-tileentity)
  ...
  - [5.10 Manual System](#510-manual-system铽丝科技手册)
  - [5.11 Advance Planner（高级计划器）](#511-advance-planner高级计划器)
  - [5.12 次元口袋（Dimensional Pocket）](#512-次元口袋dimensional-pocket)
  - [5.13 物质球解压器（Matter Ball Decompressor）](#513-物质球解压器matter-ball-decompressor)
- [6. GUI 与交互](#6-gui-与交互)
- [7. 网络包](#7-网络包)
- [8. AI 助手架构](#8-ai-助手架构)
...
`铽丝科技` 是一个 Minecraft `1.7.10` / Forge `10.13.4.1614` / GTNH 环境下的工具模组，mod id 为 `textech`。核心能力分为三层：

- 世界内监视器：`TileEntityAdvanceDataMonitor` 采集目标 TileEntity / AE2 链接器数据，客户端 TESR 渲染为图表、文本、合成状态或存储物品列表。
- AE2 链接器方块：原 Network / Storage / Crafting 三类 Link 已合并为统一 `BlockAdvanceNetworkLink` / `TileEntityAdvanceNetworkLink`，提供存储容量、指定物品数量、合成 CPU 状态和自动合成能力。
- AI / 语音助手：客户端把自然语言解析为结构化 intent，通过 Forge packet 调服务端查询 AE2、提交合成、取出物品到背包或管理计划；语音入口先 STT，再复用文本助手链路。

项目大体仍沿用 GTNH ExampleMod 构建骨架，但业务代码已经集中在 `com.imgood.textech` 包下。
...
- `dependencies.gradle`：声明运行/编译依赖。当前 shadow 包含 Vosk/JNA、PinIn（手册拼音搜索），以及 GTNH、AE2 Fluid Craft、GT5、ArchitectureCraft、BlockRenderer6343 等 dev 依赖。
...
项目采用 **双轴包组织**：标准 Forge 分层（`blocks/`、`items/`、`entity/` 等）+ 功能包（`assistant/`、`items/cell/`、`handler/`、`voice/`）。复杂子系统的共享逻辑放功能包；Block/Item/Entity/Handler/Render 仍注册在标准包，经 `loader/` 集中注册。

主要目录如下：

- `src/main/java/com/imgood/textech/`：mod 入口、proxy、配置和各业务包。
- `blocks/`：五类方块实现（含挂索节点 `BlockGrappleAnchor`），负责创建对应 TileEntity、打开 GUI、放置方向、基础交互。
- `items/`：`ItemDataImprint`、`ItemAdvanceStorageLinkCell`、`ItemAdvancePlanner`、`ItemManual`、`ItemSuperOrange`、`ItemGrappleHook`、`ItemStarryCosmosSword`、`ItemDimensionalPocket`（+ `ItemSpaceUpgradeCard` / `ItemPageUpgradeCard` / `ItemStackUpgradeCard` / `ItemInfiniteStackUpgradeCard`）；数据编织元件见 `items/cell/` 包（Item 也在 `items/cell/` 内，与挂索/手册的 items/ 模式不同）。
- `tileentity/`：监视器、统一 AE2 网络链接器与挂索节点的服务端状态、NBT 持久化、AE2 网络访问和同步逻辑。
...
- `handler/`：tick handler、SuperOrange 能力处理器、战利品处理器、玩家加入处理器、挂索服务端逻辑（节点索引、玩家状态、锚点坐标、移动队列）、至高天圣裁共享工具（伤害源、运动、落点、常量、音效）、次元口袋服务端逻辑（状态、存储、库存、升级规则）；包含 `HandlerTick` / `HandlerSuperOrange` / `HandlerLoot` / `HandlerPlayerJoin` / `HandlerGrapple` / `HandlerStarryCosmosSword` / `HandlerDataLoomCell` 等。
- `entity/`：`EntitySuperOrangeDrone`、挂索滑移实体、至高天圣裁技能实体（6 个）；不含 Render 类。
- `client/`：语音热键、按键绑定、挂索客户端输入（`HandlerGrappleClient`）、挂索客户端缓存与选点（`GrappleClientCache`、`GrappleSelectionUtil`）、次元口袋悬浮窗与客户端缓存。
- `gui/manual/`：手册章节/页面模型与 JSON 加载（已迁入 `gui/` 子包）；`ItemManual` 在 `items/`，`GuiManual` 在 `gui/guiscreen/`。
- `assistant/`：AI 助手 intent、controller、服务端执行、格式化、偏好记忆和本地计划存储。
- `assistant/ai/`：OpenAI-compatible chat client、请求选项、provider profile、stream listener（assistant 子包）。
- `voice/`：录音、STT、Vosk 嵌入模型。
- `webae/`：嵌入式 Web 控制台（NanoHTTPD HTTP 服务器 + REST API + 前端 SPA），含 `chat/` 聊天环形缓冲与 `screenshot/` 有界截图附件接收/存储；详细架构、API 端点表、网络包与子系统设计见 [WebAE 开发者手册](../webae/开发者手册.md)。
...
- `utils/`：NBT 解析、绑定数据模型、TileEntity 类型识别、网络校验、AE2 合成模板等辅助工具。
- `mixin/`：客户端 Mixin（NEI tooltip/布局、GUI 容器钩子），配置见 `mixins.textech.json`。
- `src/main/resources/assets/textech/`：lang、texture、AI 词表、嵌入式 Vosk 模型文件清单。
...
`CommonProxy.preInit()` 调用 `Config.synchronizeConfiguration()` 读取 Forge config。`LoaderCommand`：客户端注册 `/textech`（分发 `ai`/`assistant`）、`/admai`、`/admassistant`；服务端注册 `/textech`（分发 `web`/`assistant`）、`/admassistant`、`/admweb`。由 `ClientProxy.init` / `CommonProxy.serverStarting` 调用。

集中注册类：

- `LoaderBlock` 注册 `advDataMonitor`、`advNetworkLinkBlock`、`grappleAnchor`、`matterBallDecompressor`（及可选 `uiFrameworkDebug`）。
...
- `LoaderNetwork` 注册所有 packet，discriminator `0`–`23`（完整 ID 表见 `.cursor/rules/network-packets.mdc` 与 `LoaderNetwork.java`）。

新增注册项时建议延续 loader 集中入口。

## 5. 方块、物品与 TileEntity

以下名称均来自 `assets/textech/lang/`，为游戏内实际显示名（中文 / 英文）。类文件头部 `Display names` 注释与下表一致。
...
`TileEntityAdvanceDataMonitor` 持有 `dataBoundList`，每个 index 对应一个 `NBTTagCompound` 显示项。常量 `MAX_DATA_BINDINGS = 36` 限制每个监视器面的绑定条目数（`ItemDataImprint` 潜行绑定与 `GuiSubBind` 新增绑定共用；满时提示 `adm.error.data_bindings_full`）。`MonitorWidgetSpec` 会把旧存档归一化为共享契约：`kind`、`sourceKind`、`metricKey`、`title`、可选 `style`、`targetValue`、`pins`、`columns`、`sortMode`、`maxRows`、`colors`、`seriesTransform`。新逻辑以 `kind` / `renderType` 为准，仍兼容读取并回填 `dataType`、`name`、`displayName`。`columns` 缺失表示使用默认列，空列表表示明确隐藏全部列。

共享首类 `kind` 为 `statCard`、`progressBar`、`gauge`、`lineChart`、`barChart`、`pieChart`、`dataTable`；`crafting`、`storage`、`web_surface` 保留为原生专用类型。`radarChart` 仍为 WebAE-only，需要进入世界显示时走 live/snapshot surface。旧 `bar3d` / `waterfall` 作为 `barChart` alias，`diffrence` / `difference` 迁移为 `lineChart + seriesTransform=difference`。

服务端采样按到期 binding 分组执行：

1. `tile_metric`、`ae_metric`、`storage_summary`、`gt_summary` 按目标坐标分组，同一坐标的 TileEntity 只取一次 NBT，再 fan-out 给多个 binding。
2. `wireless_eu` 与 `wireless_steam` 按监视器 owner/team 查询，不伪造目标坐标；SNL 0.2.5 蒸汽只提供存量，不提供真实容量。
3. `crafting` / `storage` 继续生成 `lines` / `networkLines` 或 `storageItems`；数值源追加 `dataValues`。
4. 每次数据或小字段变更递增 `revision`，由 packet 54 `PacketMonitorBindingDelta` 只下发 binding index、revision、追加样本和少量 patch。`PacketSynTileEntity` 保留给初次/区块加载、GUI 保存或结构性配置变更等全量同步，常规采样不再每 tick 广播整份 monitor NBT。

`dataLimit` 仍兼容 2..9999 并由服务端保留完整历史；GUI 对 `>512` 只给软警告。原生渲染和 WebAE preview 使用 `MonitorDownsampleUtil`，点数预算为 `min(历史点数, 可视宽度, 240)`。折线时序的 `MonitorRenderCache` 以 `monitorPos + bindingIndex + revision` 为 key，数据/配置没变时复用 VBO，不在每帧执行 `glGenBuffers/glBufferData/glDeleteBuffers`。

每条 binding 的 `threshold` compound 固定包含 `enabled`、`operator`、`value`、`hysteresis`、`outputMin`、`outputMax`；`MonitorWidgetSpec` 为旧存档补齐 disabled 配置，且不读取 `targetValue`、`dataLimit` 或样式推导阈值。`MonitorThresholdRuntime` 仅在内存保存最近有效服务端样本、采样 tick 和迟滞 active 状态，均不写入 TE NBT。新鲜度按 `max(2 × intervalTicks, 200 ticks)` 计算；到期在常规服务端 tick 主动归零。多 binding 的 strong 输出按 OR 聚合，weak 输出取最大值；Block 六面和 comparator override 只读 TE 聚合缓存，不触发采样。只有 weak/strong 聚合值变化时才 `markDirty()` 并通知邻居/比较器。

`dataType=webae_dashboard` 是被动数据类型，`updateEntity()` 会直接跳过，不解析坐标、不读取目标 TE、也不新增服务端 Tick 工作。其 `renderType=web_surface` 与数据类型解耦，`webSurfaceMode` 可为 `dashboard_live`（发布布局后的直播贴图）、`dashboard_snapshot`（旧绘制列表快照）或 `live_url`（任意 http(s) URL，需客户端 MCEF）。

主路径：WebAE「导出到游戏显示器」发布布局到 `TeXTech/WebAE/displays/`、复制绑定 JSON，并 `POST /api/display/{id}/frame` 上传浏览器当前视口 JPEG（`browser-jpeg`，与网页一致；页签打开可续推）。兜底：① 客户端 montoyo MCEF 且 `webSurfaceUseMcef=true` 时本机 OSR（`mcef`）；② 主机 Chrome/Edge 无头截 `/embed/dashboard?...&capture=1`（`spa-jpeg`；可配 `webDisplayChromePath` / `WEBAE_CHROME_PATH`；再失败 `server-html`）。**不再**把精简 `render.html` 或 AWT 绘制列表当作直播成功帧（`webSurfaceAllowLiveSnapshotFallback` 默认 false）。发布响应尽量带 `webaeOrigin`。MC Tick 不跑浏览器。显式 `dashboard_snapshot` 仍是伪 UI。每台监视器最多 8 个网页绑定。

客户端 `WebSurfaceSourceRouter`：可选 MCEF → CDP 占位 → HttpFrame（识别 `browser-jpeg`/`spa-jpeg`/`server-html`）；直播失败不静默回退 Snapshot。GUI 状态行显示帧源与错误码；`GET /api/display/{id}/frame-status` 可查诊断。

客户端渲染时优先根据显示项中的 `renderType`，缺失时回退 `dataType`。`LoaderRender` 已注册七类共享 renderer，折线时序共用缓存基座；`crafting`、`storage`、`web_surface` 继续由专用 renderer 处理。进度条/仪表的上限顺序是真实 source max → 0..100 百分比指标 → `targetValue>0`；无真实容量且无 target 的蒸汽等绝对量退化为纯数值/趋势，不显示伪百分比。要新增一种显示类型，通常需要：
...
`ItemDataImprint` 用于映录目标方块并保存 TileEntity 的 NBT 快照，再绑定到数据监视器；也可记录 GT 机器坐标并批量绑定（`GtMachineBinding`）。**方块映录**（`boundPos` / `boundTE`）与 **GT 机器列表**（`gtMachineList`）互斥：写入一侧会清除另一侧。潜行 + 右键空气：有载荷则清除，无载荷则按 `scanRadius`（潜行+滚轮 8/16/32/64）扫描附近 GT。潜行 + 右键监视器时按当前唯一载荷绑定；旧版双载荷物品绑定时保留 GT 列表并清除冲突方块映录。方块绑定时通过 `TileEntityAdvanceDataMonitor.findLowestFreeBindingIndex()` 分配槽位，达到 `MAX_DATA_BINDINGS`（36）则拒绝并提示 `adm.error.data_bindings_full`。与数据编织元件（织尘/织形/涌流）不同，映录器只观测与记录，不会从 AE 网络编织物质。GUI handler 的 `NBT_VIEWER_GUI_ID` 会读取手持物品上的 `boundTE`，通过 `NBTJsonParserHelper` 转成 JSON 后打开 NBT 查看器。

### 5.3 高级网络链接器（Advanced Network Linker）

核心类：`BlockAdvanceNetworkLink`、`TileEntityAdvanceNetworkLink`、`ContainerAdvanceStorageLink`、`GuiAdvanceStorageLink`、`GuiSubLinkDisplayTypeSelect`、`GuiSubAEAdvanceNetworkLink`、`GuiSubAEAdvanceCraftingLink`、`GuiSubAEAdvanceStorageLink`、`ItemAdvanceStorageLinkCell`、`AssistantServerServices`。

> 原 `BlockAdvanceCraftingLink` / `BlockAdvanceStorageLink` / 独立 `TileEntityAdvanceNetworkLink` 统计 TE 已合并。`LoaderTileEntity` 通过 legacy mapping 将旧 TE ID 重定向到 `TileEntityAdvanceNetworkLink`。

`TileEntityAdvanceNetworkLink` 继承 AE2 `AENetworkTile`，实现 `IInventory` 与 `IOwnableTile`，要求频道，连接类型 `SMART`。单 TE 集成三类能力：

**网络存储统计**：遍历 `TileDrive` / `TileChest` 等，统计物品/流体总字节、已用字节、总类型、已用类型；订阅 `MENetworkCellArrayUpdate` 与 `MENetworkStorageEvent`。

**存储显示**（36 槽，仅 `ItemAdvanceStorageLinkCell`）：按分区标记统计 AE 网络存量（精确 / Fuzzy / Inverter / Ore Filter / 流体标记）。`createStorageItemsSnapshot()` 组装 `NBTTagList` 供 `StorageInfoRenderer`；`sampleStorageDeltasIfNeeded()` 每 20 tick 采样增量。

> **与数据编织元件的区别**：存储统计只计入「高级存储链接元件」所标记物品/流体/源质在 AE 网络中的聚合数量。数据编织元件内部 `dataLoomItemAccum` / `dataLoomFluidAccum` **不纳入**统计。

**合成 CPU 监控**：接入 crafting grid，统计 CPU 数量、忙碌数、存储、协处理器与每 CPU 快照；订阅 `MENetworkCraftingCpuChange`。监视器 `dataType=crafting` 时读取 summary 或模板。

**交互**：右键 `openGui` → `GuiAdvanceStorageLink`（`ADM_STORAGELINK_ID`）；Shift+右键聊天输出网络 + 合成综合状态；监视器绑定时经 `GuiSubLinkDisplayTypeSelect` 选择显示类型（`adm.button.link_display_*`）。

AI 助手服务端执行依赖玩家附近的高级网络链接器（默认 32 格，`[assistant] linkSearchRadius` 4–128）：查询候选、样板、提交/取消合成、取出物品均经该 TE 进入 AE2 网络。

### 5.4 Super Orange（超能砂糖桔）

核心类：`ItemSuperOrange`、`GuiSuperOrangeConfig`、`PacketSuperOrangeConfig`、`EntitySuperOrangeDrone`、`HandlerSuperOrange`、`HandlerTick`、`HandlerLoot`。

**操作与 NBT**

| 操作 | 行为 |
|------|------|
| 右键（服务端） | 切换 `droneEnabled` NBT，开关无人机与投掷物免疫 |
| 潜行+右键（客户端） | 打开 `GuiSuperOrangeConfig`（`GuiHandler.SUPER_ORANGE_GUI_ID = 8`） |
| 设置 GUI 保存 | `PacketSuperOrangeConfig`（ID 22）→ 服务端写入铭牌、`matterBallEnabled`、`pickupMatterBallEnabled`、`dropMatterBallEnabled`、`dropMultiplier` |

`HandlerTick` 在玩家 tick 中调用 `EntitySuperOrangeDrone.spawnForPlayer()`；当物品 NBT 关闭无人机或服务端 `droneEnabled=false` 时移除该玩家所有无人机。

`HandlerSuperOrange` 处理投掷物免疫、挖掘掉落倍率/物质团簇合并，以及拾取/丢弃物品的物质团簇合并（分别受 `pickupMatterBallEnabled` / `dropMatterBallEnabled` 控制）。
...
配置由 `[superOrange]` 分节控制（含 `dropMultiplierMax` 限制 GUI 倍率上限）。详见 [用户手册 §3.10](../player/用户手册.md#310-超能砂糖桔)。
...
**注册流程**：
1. `LoaderItem.registerItems()` 注册五件元件与两张编织增幅卡。
2. `TeXTech.postInit()` 调用 `DataLoomCellHandler.register()`。
...
- 公共 tooltip 注明：元件内 `dataLoomItemAccum` / `dataLoomFluidAccum` 存量**不计入**高级网络链接器（`TileEntityAdvanceNetworkLink`）的存储监控统计。

**与高级网络链接器的关系**：
- 高级网络链接器通过 `IStorageGrid` 按「高级存储链接元件」分区标记统计网络存量。
- 数据编织元件的内部累积存储独立于链接器存储统计；监视器绑定链接器 `dataType=storage` 时不会反映编织元件内部的填充情况。
...
### 5.10 Manual System（铽丝科技手册）

注册名 `manual`（lang：`item.manual.name`）。

核心类：`ItemManual`、`gui/manual/ManualDataLoader`、`ManualChapter`、`ManualPage`、`GuiManual`、`renders/ManualPageRenderer`；搜索与高亮：`ManualSearchIndex`、`ManualSearchUtil`（PinIn + 可选 NEC `NecharUtils` 反射）、`ManualTextHighlighter`。

JSON 位于 `assets/textech/manual/`；`HandlerPlayerJoin` 首次加入赠送手册。

`GuiManual` 布局：左侧顶栏搜索框、可滚动章节列表（滚动条含顶/底跳转按钮）、右侧分页内容区。搜索对章节标题与全部页面正文建立索引（`ManualSearchIndex`），空格分隔 AND；拼音匹配内置 shadow **PinIn 1.6.0**，若加载 **NotEnoughCharacters** 则优先走其 `NecharUtils.contain`。详见 [用户手册 §3.12](../player/用户手册.md#312-铽丝科技手册)。
...
**包路径：** `com.imgood.textech.items.PlannerEntry`
...
**包路径：** `com.imgood.textech.items.PlannerMergeMode`
...
**包路径：** `com.imgood.textech.items.ItemAdvancePlanner`
...
**基类选型**（`gui/custom/`，详见 [ui-framework.md](ui-framework.md) 与 [new-feature-checklist.md](new-feature-checklist.md)）：

| 基类 | 用途 | 示例 |
|------|------|------|
| `ADM_GuiScreen` | 复杂配置/聊天/计划器 | `GuiAIChat`、`GuiAdvancePlanner` |
| `AdmItemConfigScreen` | 小型物品/节点 NBT 配置窗 | `GuiGrappleHookConfig`、`GuiGrappleAnchorConfig`、`GuiSuperOrangeConfig` |
| `AbstractMonitorSubGui` | 监视器 per-binding 子页 | `GuiSubAEAdvanceStorageLink` 等 5 个 `GuiSub*` |
| `ADM_UiContainer` | 新容器 GUI（9-slice + 图集） | `GuiMatterBallDecompressor` |
| `AdmGuiTextures` | 共享纹理常量 | 所有 ADM GUI |

##### 7.1 GuiAdvancePlanner

**包路径：** `com.imgood.textech.gui.guiscreen.GuiAdvancePlanner`
...
编辑状态 (editingSlotIndex >= 0, editingField != null)
  ...
  │   （新建时空确认不创建条目；编辑已有条目时空确认则 removeEntry 删除，等同 X 按钮）
...
**包路径：** `com.imgood.textech.gui.guiscreen.GuiPlannerMergeConfirm`
...
2. `LoaderItem.registerItems()` 设置：
   ...
   - `setTextureName("textech:advance_planner")` — 对应 `textures/items/advance_planner.png`
...
计划器 HUD 已在 `renders/PlannerHudRenderer.java` 中实现，并在 `ClientProxy.init()` 中通过 `MinecraftForge.EVENT_BUS.register(new PlannerHudRenderer())` 注册。

**渲染行为：**

- 监听 `RenderGameOverlayEvent.Post`，元素类型为 `TEXT`（非 `ALL`）。
- 扫描玩家背包（快捷栏优先）中第一个启用 HUD 的高级计划器（`ItemAdvancePlanner.isHudEnabled`），不要求手持。
- 按 `PlannerMergeMode.BY_INDEX` 排序，**仅显示未完成**条目，最多 `getHudMaxDisplay()` 条。
- 支持 NBT 中的自定义标题、位置比例（`getHudPosX` / `getHudPosY`）、缩放（`getHudScale`）、文本宽度（`getHudWidth`）；标题缺省为 `adm.planner.title`。
- 每行显示 `#slotIndex` 与自动换行后的任务文本；空文本显示 `...`。

**启用/关闭：** 潜行 + 右键计划器切换 HUD，或通过 GUI（§9.5）；HUD 开关与布局参数存于计划器 NBT（§6.3）。

**扩展 HUD 时：** 修改 `PlannerHudRenderer` 或复用同包 `HudRenderUtil`；新增 NBT 字段时同步更新 `ItemAdvancePlanner` 读写 API 与 lang 键。
...
- 中文：`src/main/resources/assets/textech/lang/zh_CN.lang`
- 英文：`src/main/resources/assets/textech/lang/en_US.lang`
...
# HUD 相关（已定义于 en_US.lang / zh_CN.lang，键名 adm.planner.hud_*）
...
1. Add Chinese key-value pairs in `assets/textech/lang/zh_CN.lang`.
2. Add matching English key-value pairs in `assets/textech/lang/en_US.lang`.
3. 在代码中使用 `I18n.format("adm.planner.xxx")` 或 `net.minecraft.client.resources.I18n.format(...)` 引用。
4. 如需带参数，使用 `%s` 占位符，在 `format()` 调用时传入参数。

---

### 5.12 次元口袋（Dimensional Pocket）

> 玩家向说明见 [用户手册 §3.13](../player/用户手册.md#313-次元口袋)

#### 架构概览

| 层级 | 类 | 职责 |
|------|-----|------|
| 状态 / 持久化 | `PocketState` / `PocketStore` / `PocketInventory` | 按玩家 UUID 维护槽位与升级；NBT 压缩写入 `<世界存档>/textech/pocket-<uuid>.dat` |
| 共享交互 | `PocketSlotInteraction` | GUI 与悬浮窗共用的左/右键、Shift 快速取出（每次最多一组） |
| GUI | `ContainerPocketStorage` / `GuiPocketStorage` | 存储主界面；`slotClick` / `transferStackInSlot` 委托 `PocketSlotInteraction` |
| 悬浮窗 | `PocketOverlayHandler` / `GuiPocketOverlay` | Shift+右键开关；tooltip 经 `GuiScreenTooltipAccess` 走 Forge `renderToolTip` |
| 客户端缓存 | `PocketClientCache` | 接收 `PacketPocketSync`，供悬浮窗渲染 |
| 网络 | `PacketPocketAction` / `PacketPocketSync` | C→S 存取/翻页/开关浮窗；S→C 同步状态 |
| 角标格式 | `PocketStackSizeFormat` / `PocketStackOverlayRenderer` / `MixinPocketDrawSlotOverlay` | 大数量 K/M/G 显示（对齐 GT Not Leisure 便携无尽箱思路） |

**打开约定**：存储 GUI 与升级配置 GUI 必须由**服务端** `player.openGui` 触发，否则 `openContainer` 会停留在 `ContainerPlayer`，点击会落到错误容器。

#### 已修复问题（2026-06）

以下问题已于 2026-06-30 修复，记录根因与修复方案备查。

| # | 现象 | 根因 | 修复方案 |
|---|------|------|----------|
| 1 | **重启存档后**，口袋内物品 `stackSize` 变为 **1** | 原版 `ItemStack.writeToNBT(slotTag)` 将 `stackSize` 以 `setByte("Count", (byte)stackSize)` 写入 NBT。在 GTNH/Forge 环境中，`ItemStack.loadItemStackFromNBT` 可能被打补丁改为 `getInteger("Count")` 读取，导致类型不匹配。JSON 持久化路径中 `tag.toString()` → `JsonToNBT.func_150315_a()` 返回的 Count 标签类型与加载期读取方法不一致，Count 丢失默认回退到 1。 | `PocketState` 新增 `writeItemStackToNBT` / `readItemStackToNBT` 方法，写入时用 `setInteger("Count", stack.stackSize)` 显式保存为 int，读取时通过 `readStackCount` 兼容 byte/short/int 三种标签类型。 |
| 2 | **悬浮窗槽位角标始终显示 0** | 同上：`PacketPocketSync.toBytes()` 中沿用 `writeToNBT` 写入 byte 类型的 Count，客户端 `fromBytes()` → `loadItemStackFromNBT` 读取时 Count 为 0。存储 GUI 不受影响（因走原版 `S2FPacketSetSlot` 用 `buf.writeByte` 直接传输）。 | `PacketPocketSync.toBytes/fromBytes` 统一改用 `PocketState.writeItemStackToNBT` / `readItemStackFromNBT`，确保网络传输中 Count 为 int 类型。 |
| 3 | **重启存档后口袋数据丢失**（JSON→NBT 存储迁移） | 旧版将口袋数据以 JSON 格式存储于 `config/textech/pocket-<uuid>.json`。新版改用 `CompressedStreamTools` 压缩 NBT 写入 `<世界存档>/textech/pocket-<uuid>.dat`，按世界存档隔离存储。 | `PocketStore` 重构为 `CompressedStreamTools` 读写；`铽丝科技` 新增 `WorldEvent.Load` 监听器设置世界存档目录。迁移期间若新路径无文件，自动降级读取旧 `advancedatamonitor/` 或 JSON 路径。 |

**影响文件**：`handler/PocketState.java`、`handler/PocketStore.java`、`network/packet/PacketPocketSync.java`、`TeXTech.java`。

---

### 5.13 物质球解压器（Matter Ball Decompressor）

> 玩家向说明见 [用户手册 §3.14](../player/用户手册.md#314-物质球解压器)

核心类：`BlockMatterBallDecompressor`、`TileEntityMatterBallDecompressor`、`ContainerMatterBallDecompressor`、`GuiMatterBallDecompressor`、`MatterBallDecompressorUpgrades`、`MatterBallClusterUtil`、`AeUpgradeSpeedUtil`、`PacketMatterBallDecompressorToggle`、`RenderMatterBallDecompressor`。

**架构**

- 继承 `AENetworkTile`，`GridFlags.REQUIRE_CHANNEL`，智能线缆（`AECableType.SMART`）。
- 左侧 9 槽 `inputInv` 仅接受 Avaritia 物质团簇；右侧 81 槽 `bufferInv`（9×9 GUI）。
- 4 槽 `MatterBallDecompressorUpgrades`：仅 `Upgrades.SPEED` / `Upgrades.SUPERSPEED`（各最多 4 张）。
- 服务端 tick：`decompressOneItem()` 从团簇提取 1 个物品；`outputToNetwork=true` 时注入 `IStorageGrid`，失败或缓冲模式时写入缓存。
- **阻塞模式**（`blockMode`）：仅在 `outputToNetwork=false` 时生效；缓存有任意物品则暂停解压。
- 速率：`Config.matterBallDecompressorItemsPerSecond`（默认 16/s）× `AeUpgradeSpeedUtil.getSpeedMultiplier()`。

**GUI / 网络**

- `GuiHandler.MATTER_BALL_DECOMPRESSOR_GUI_ID = 9`；模式切换经 `PacketMatterBallDecompressorToggle`（ID 23，`KIND_OUTPUT_MODE` / `KIND_BLOCK_MODE`）。

**合成**

- `LoaderRecipe.registerAssemblyLineRecipes()`：UHV 装配线，扫描高级网络链接器 90s @ UV，120s @ UHV；材料见 [用户手册 §3.12](../player/用户手册.md#312-物质球解压器)。

**配置**：`[matterBallDecompressor] itemsPerSecond`（见 `ConfigMatterBallDecompressorLoader`）。

---

> 本文档基于 TeXTech 模组源码编写。如代码发生变更，请以实际源码为准。

## 6. GUI 与交互

`GuiHandler` 当前定义十一个 Forge GUI id：

- `0`（`NBT_VIEWER_GUI_ID`）：NBT Viewer，只在客户端打开。
- `1`（`ADM_MAIN_GUI_ID`）：主监视器 GUI。
- `2`（`ADM_STORAGELINK_ID`）：统一高级网络链接器存储 GUI，服务端返回 `ContainerAdvanceStorageLink`，客户端返回 `GuiAdvanceStorageLink`。
- `3`（`MANUAL_GUI_ID`）：模组手册 `GuiManual`。
- `4`（`GRAPPLE_ANCHOR_GUI_ID`）：挂索节点配置 `GuiGrappleAnchorConfig`。
- `5`（`GRAPPLE_HOOK_GUI_ID`）：挂索器配置 `GuiGrappleHookConfig`。
- `6`（`POCKET_CONFIG_GUI_ID`）：次元口袋升级配置 `GuiDimensionalPocketConfig`（须服务端 `openGui`）。
- `7`（`POCKET_STORAGE_GUI_ID`）：次元口袋存储 `GuiPocketStorage`（须服务端 `openGui`；右键口袋主入口）。
- `8`（`SUPER_ORANGE_GUI_ID`）：超能砂糖桔设置 `GuiSuperOrangeConfig`（潜行+右键客户端打开，保存经 `PacketSuperOrangeConfig`）。
- `9`（`MATTER_BALL_DECOMPRESSOR_GUI_ID`）：物质球解压器 `GuiMatterBallDecompressor` / `ContainerMatterBallDecompressor`。
- `10`（`UI_FRAMEWORK_DEBUG_GUI_ID`）：UI 框架调试方块 → `GuiUiFrameworkDebug` / `ContainerUiFrameworkDebug`（需 `[debug] uiFrameworkBlock=true`）。

**容器 UI 框架**（9-slice 图集、`UiTheme`、物质球解压器为首个调试验证界面）：详见 [ui-framework.md](ui-framework.md)。

主监视器 GUI 以 `GuiMainAdvanceDataMonitor` 为入口，子页面负责绑定目标、配置统一高级网络链接器（`GuiSubLinkDisplayTypeSelect` 选择网络/存储/合成显示类型）、颜色和显示变换。AI 聊天与 AI 设置是独立 `GuiScreen`，由主界面按钮、命令或语音热键打开。

GUI 修改 TileEntity 时要注意 side：客户端负责编辑界面和发包，服务端负责可信状态更新并 `markDirty()` / `markBlockForUpdate()`。不要只改客户端 TileEntity，否则重开世界或服务端同步后会丢失。

## 7. 网络包

所有 packet 共用 `TeXTech.ADMCHANEL`，注册在 `LoaderNetwork.registerNetWorks()`（postInit）。

**完整 ID 表与新增接入步骤**：见 [`.cursor/rules/network-packets.mdc`](../../../.cursor/rules/network-packets.mdc)（与源码保持同步，下一可用 ID = 55）。

摘要：

| 范围 | 子系统 |
|------|--------|
| 0–5 | 映录器 / 监视器 TE 同步 / 存储链接器数量 |
| 6–7 | AI 助手 C→S / S→C |
| 8–10 | 计划器同步与合并 |
| 11–18 | 挂索动作、配置、路径 |
| 19–21 | 次元口袋 |
| 22–23 | 超能砂糖桔 / 物质球解压器 |
| 24–25 | AI 助手菜单状态 |
| 26–50 | WebAE 配方/图标/Token/世界地图/告警 HUD |
| 51–52 | 游戏窗口截图 24 KiB 分片上传 / 终态确认 |
| 53 | 监视器网页表面按哈希请求、单绑定上传与 ACK（同 ID 双向） |
| 54 | 监视器 binding revision/追加样本/小字段 patch 的 S→C 增量同步 |
...
- 服务端 handler 中涉及世界和 TileEntity 修改时，保持在服务端线程上下文中执行；各 packet Handler 经 `PacketHandlers.runOnServer()` 切回服务端 tick（内部仍用 `HandlerTick.enqueueServerTask`）。

**统一信任边界**：Forge 1.7.10 custom payload 的硬上限是 32767 bytes；项目内跨边界 packet 使用 30000 bytes 包体预算，二进制正文通常按 24 KiB 非空分片。所有 byte、string、VarString 字段都必须在读取前检查长度与剩余字节，并拒绝尾随数据；文本使用严格 UTF-8 解码，禁止用替换字符吞掉 malformed/unmappable 字节。Forge 1.7.10 的 NBT 使用 signed-short 压缩长度 framing（`-1` 表示 null），解压必须配合有限 `NBTSizeTracker`，同时限制压缩 payload；assistant gzip NBT 还限制为 24 KiB 压缩体、2 MiB 展开量。JSON 必须校验根类型、列表数量、字符串长度、控制字符和枚举/索引范围；非法数据应安全丢弃，不能把原始 payload 回显到错误消息。

C→S handler 的 actor 必须来自 `ctx.getServerHandler().playerEntity`；客户端上报的 player UUID、owner UUID、actor UUID 或 network ID 只能作为待校验输入，不能直接授权。所有世界/TileEntity/AE mutation 通过 `PacketHandlers.runOnServer()` 调度，ACK 也在服务端主线程发送；handler 需要处理截断、非法 UTF-8、超限 NBT/JSON 和非法枚举而不抛出未捕获异常。`utils/NetworkPacketCodec` 集中实现 framing/size 检查，`WebAeBinaryTransfer.SequentialAssembler` 提供有界、带 TTL 的严格顺序重组；世界地图 snapshot key 列表在 30 KiB 内按 offset 分页，客户端完整拉完一页链后才提交版本。`webae/worldmap/WorldMapPacketAuthorization` 负责 owner/network/dimension/chunk/layer 边界；Dynmap provider/root 还负责真实文件路径 containment，不能绕过路由或授权直接读任意文件。

## 8. AI 助手架构

AI 助手由客户端解析、服务端执行。**完整架构、JSON schema、必改文件清单与测试指南**见 [AI 助手开发指南](../ai-assistant/开发指南.md) 与 `.cursor/rules/ai-assistant.mdc`。
...
flowchart TD
    ...
    B --> C{AI 可用?}
    C -->|否| D[AssistantIntentService fallback]
    C -->|是| E[AssistantAiIntentService → DeepSeekChatClient]
...
关键类：`AssistantController`（客户端总控）、`AssistantAiIntentService` + `AssistantIntentService`（AI / fallback 解析）、`PacketAssistantAction` / `PacketAssistantResponse`、`AssistantServerServices`（服务端 AE2 执行）。

新增 assistant 能力时通常需同步修改 intent 类型、JSON parser、prompt schema、lexicon、controller 映射、packet 与 `AssistantIntentParserSuite` 回归用例。详见开发指南 Part B。

## 9. 普通 AI 聊天与配置

`DeepSeekChatClient` 是 OpenAI-compatible HTTP client，普通聊天和 AI intent 抽取都复用它。启用联网搜索时，`WebSearchService` 先在客户端检索网页并注入上下文，再调用 LLM（不依赖 provider-native search 参数）。配置在 `Config` 中，主要 category：

- `ai`：base URL、API key、model、network enabled、内置联网搜索（引擎/Key/SearXNG URL/结果数/降级）、streaming、timeout、max tokens、temperature、privacy flag、debug logging。客户端保存在 `config/textech/ai-client-local.cfg`。
...
- 方块 / 物品外观：`RenderAdvanceDataMonitor`、`RenderAdvanceDataMonitorBlockItem`、`RenderAdvanceNetworkLink`、`RenderAdvanceNetworkLinkBlockItem`、各 item renderer，由 `LoaderRender` 绑定。
- 监视器内容：实现 `IADMRender`，通过 `RenderController` 按 type 分发。

`IADMRender.render(NBTTagCompound nbt, double x, double y, double z, int facing, int bindingIndex)` 直接消费显示项 NBT，因此渲染器和采集逻辑之间的契约是 NBT 字段名。`bindingIndex` 供按需内容渲染器定位单绑定正文；改字段名时必须同步 GUI 默认值、TileEntity 采集、NBT 持久化、同步包和 renderer。
...
- TileEntity NBT：监视器显示项、统一 AE2 链接器缓存与 36 槽 inventory。
...
- 资源文件：`assets/textech/config/assistant-lexicon.json` 是 fallback parser 词表。
...
- 高级网络链接器：接入 AE2 网络，插入/移除存储元件，Shift+右键确认综合状态；监视器分别验证网络图表、存储显示、合成文字三种 `dataType` 绑定。
...
所有 debug 开关在 `config/textech/textech.cfg`，**默认均为 false**：
...
3. 新增/修改文本时同步 `assets/textech/lang/en_US.lang` + `zh_CN.lang`
