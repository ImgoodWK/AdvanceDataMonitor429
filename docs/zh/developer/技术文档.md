...
`dataType=webae_dashboard` 是被动数据类型，`updateEntity()` 会直接跳过，不解析坐标、不读取目标 TE、也不新增服务端 Tick 工作。其 `renderType=web_surface` 与数据类型解耦，`webSurfaceMode` 可为 `dashboard_live`（发布布局后的直播贴图）、`dashboard_snapshot`（旧绘制列表快照）或预留的 `live_url`。

主路径：WebAE「导出到游戏显示器」发布当前仪表盘布局+设置到 `TeXTech/WebAE/displays/`，复制 `textech-webae-display-binding` v1（含 `displayId` / `viewToken`）。监视器导入后客户端按距离混合刷新，从 `GET /api/display/{id}/frame.jpg` 拉取 JPEG；WebAE 主机用系统 Chrome/Edge 无头截帧（`DisplayCaptureService`），MC Tick 不跑浏览器。无截帧能力时仍可导入旧 `textech-webae-display-snapshot` 绘制列表，由 `WebSurfaceClientCache` AWT 栅格化。`writeSyncNBT()` 继续剥离大正文；packet 53 负责绑定上传与快照正文按需拉取。每台监视器最多 8 个网页绑定、快照正文合计最多 128 KiB。

客户端 `WebSurfaceSourceRouter` 按优先级尝试 MCEF（软依赖占位）→ 本机 CDP（占位）→ HttpFrame → Snapshot。稳定帧路径只绑定纹理并绘制一个四边形。
