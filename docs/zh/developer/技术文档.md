...
主路径：WebAE「导出到游戏显示器」发布布局到 `TeXTech/WebAE/displays/` 并复制绑定 JSON。成功监视器画面只有两种真实网页像素：① 客户端安装 montoyo MCEF 且 `webSurfaceUseMcef=true` 时本机 OSR（`mcef`）；② 主机用系统 Chrome/Edge 无头截真实 `/embed/dashboard?...&capture=1`（`DisplayCaptureService` → `spa-jpeg`）。**不再**把精简 `render.html` 或 AWT 绘制列表当作直播成功帧（`render.html` 仅调试；`webSurfaceAllowLiveSnapshotFallback` 默认 false）。发布响应尽量带上浏览器 Origin 作为 `webaeOrigin`。MC Tick 不跑浏览器。显式 `dashboard_snapshot` 仍是伪 UI。每台监视器最多 8 个网页绑定。

客户端 `WebSurfaceSourceRouter`：可选 MCEF → CDP 占位 → HttpFrame；直播失败不静默回退 Snapshot。GUI 状态行显示帧源与错误码；`GET /api/display/{id}/frame-status` 可查主机截帧诊断。
