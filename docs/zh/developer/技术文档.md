...
`dataType=webae_dashboard` 是被动数据类型，`updateEntity()` 会直接跳过，不解析坐标、不读取目标 TE、也不新增服务端 Tick 工作。其 `renderType=web_surface` 与数据类型解耦，当前 `webSurfaceMode=dashboard_snapshot`，以后可在不改监视器空间变换 NBT 的前提下扩展其他网页内容源。

WebAE 前端 `utils/dashboardGameDisplayExport.ts` 从当前已渲染的 Dashboard DOM 生成 `textech-webae-display-snapshot` v1：视口 + 最多 600 个 `rect` / `ellipse` / `text` / `polyline` 绘制元素，原始 JSON 主动裁剪到约 90 KiB。`WebDashboardSnapshotCodec` 在客户端导入和服务端保存时再次校验，要求 GZIP 后不超过 24 KiB。正文只写入服务端 TE 持久化 NBT；`writeSyncNBT()` 会从常规 TE 同步中剥离正文，只发送哈希、视口、元素数和尺寸元数据。`PacketMonitorWebSurface`（ID 53，同 ID 双向）负责单绑定上传/ACK和 64 格内客户端按哈希请求正文。每台监视器最多 8 个网页绑定、正文总计最多 128 KiB。

客户端 `WebSurfaceClientCache` 使用单个低优先级 daemon worker 把绘制列表栅格化为 256/512/1024px `DynamicTexture`；GL 上传仍在渲染线程，纹理使用 8 项/约 6M 像素 LRU。稳定帧路径只绑定纹理并绘制一个四边形，不重复解析 JSON，也不请求 WebAE HTTP。
