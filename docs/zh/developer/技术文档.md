...
`CommonProxy.preInit()` 调用 `Config.synchronizeConfiguration()` 读取 Forge config。`ClientProxy.init()` 在客户端注册 `/textech`、`/admai`、`/admassistant`、语音热键和热键事件对象；服务端注册 `/textech`、`/admassistant`、`/admweb`。
...
**完整 ID 表（0–36）与新增接入步骤**：见 [`.cursor/rules/network-packets.mdc`](../../.cursor/rules/network-packets.mdc)（与源码保持同步，下一可用 ID = 37）。
