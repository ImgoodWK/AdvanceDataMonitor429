...
`TileEntityAdvanceDataMonitor` 持有 `dataBoundList`，每个 index 对应一个 `NBTTagCompound` 显示项。常量 `MAX_DATA_BINDINGS = 36` 限制每个监视器面的绑定条目数（`ItemDataImprint` 潜行绑定与 `GuiSubBind` 新增绑定共用；满时提示 `adm.error.data_bindings_full`）。显示项里保存目标坐标、字段名、采样间隔、颜色、缩放、旋转、数据类型等；其中 `dataLimit` 为单条图表保留的数据点数，默认 100，与绑定条目上限无关。服务端 `updateEntity()` 按每个显示项的 `interval` 采样：

1. 解析绑定坐标。
2. 找到目标 TileEntity。
3. 如果目标是统一高级网络链接器且 `dataType=crafting`，写入 `lines` 或 `networkLines`。
4. 如果目标是统一高级网络链接器且 `dataType=storage`，写入 `storageItems`。
5. 如果目标是统一高级网络链接器且 `dataType=line`（默认网络图表），根据字段读取 AE2 网络统计值，可选择转百分比。
...
客户端渲染时优先根据显示项中的 `renderType`，缺失时回退 `dataType`，分发到 `LineChartRenderer`、`CraftingInfoRenderer`、`StorageInfoRenderer`、`WebSurfaceRenderer`。要新增一种显示类型，通常需要：
...
**完整 ID 表与新增接入步骤**：见 [`.cursor/rules/network-packets.mdc`](../../.cursor/rules/network-packets.mdc)（与源码保持同步，下一可用 ID = 54）。
