...
> **项目**: AdvanceDataMonitor (GTNH 1.7.10 / Forge)  
...
| 占位贴图 | `textures/items/AdvanceDataMonitor.png` |
...
| maxStackSize | 1 |
| 占位贴图 | `advancedatamonitor:advance_planner` |
...
- 模型：`model/AdvanceDataMonitor2.obj`  
- 纹理：`textures/model/AdvanceDataMonitor.png`  
...
**分类**: `grapple`（`config/AdvanceDataMonitor.cfg`）
...
| 资源 | 当前占位 |
|------|---------|
| 方块 TESR | `AdvanceDataMonitor2.obj` + 纹理 |
...
7. **未加工作台合成配方**  
8. ~~**未加手册 JSON 章节**~~（已在高级数据监视器手册「方块与物品」章节补充挂索节点/挂索器页面）  
