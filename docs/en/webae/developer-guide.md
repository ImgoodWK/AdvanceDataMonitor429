...
- **Dashboard customization**:
  ...
  - **In-game display binding**: publish + `textech-webae-display-binding` v1. Successful frames: host `DisplayCaptureService` captures real `/embed/dashboard?...&capture=1` (`spa-jpeg`), or client MCEF OSR (`mcef`). **Not** successful live frames: `render.html` or AWT snapshot (`webSurfaceAllowLiveSnapshotFallback` defaults false). Publish warms `frame.jpg`; failures expose error codes. Client router: MCEF → CDP stub → HttpFrame. GUI shows frame source + error; `GET /api/display/{id}/frame-status` returns `hasFrame`/`inFlight`/`error`. Optional `webSurfaceUseMcef` (default true) + configurable MCEF pool/distance. Publish writes browser Origin into `webaeOrigin` when available.
