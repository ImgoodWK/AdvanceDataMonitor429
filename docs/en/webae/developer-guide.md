...
- **Dashboard customization**:
  ...
  - **In-game display binding**: publish + `textech-webae-display-binding` v1. Default lazy path: host `DisplayCaptureService` headless-captures real `/embed/dashboard?...&capture=1`, falling back to `render.html` if blank; clients pull `frame.jpg`. Optional `webSurfaceUseMcef` (default true; falls back to host capture when MCEF is absent) + montoyo MCEF OSR. Publish writes browser Origin into `webaeOrigin` when available.
