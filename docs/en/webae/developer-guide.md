...
- **Dashboard customization**:
  ...
  - **In-game display binding**: the primary export publishes the current dashboard layout+settings via `/api/display` and copies a `textech-webae-display-binding` v1 (`dashboard_live`) for the monitor. Embed route: `/embed/dashboard/:id`. Live frames come from WebAE host capture at `/api/display/{id}/frame.jpg`. On publish failure it can fall back to the legacy DOM drawing-list snapshot (`dashboardGameDisplayExport.ts` / `textech-webae-display-snapshot`). Tokens, AI keys, webhook URLs, and QQ/mail secrets are excluded; no MC-tick browser/polling
