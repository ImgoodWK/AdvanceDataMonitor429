...
- **Web-assisted steps**: item/fluid submit and retrieval detect; crafting/kill/dimension steps are in-game only. Click a single step icon to submit that step; **Chain submit** walks prerequisites in topological order (configurable), shows missing materials, and allows **submit available only** or **craft and submit all**. The submit panel shows AE stock vs requirement (`AE available/required`); **Craftable** appears only when a matching AE pattern exists and the full material chain can be satisfied.
