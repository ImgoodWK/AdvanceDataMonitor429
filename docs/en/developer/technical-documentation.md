...
- [5. Blocks, Items, and TileEntities](#5-blocks-items-and-tileentities)
  ...
  - [5.13 Matter Ball Decompressor](#513-matter-ball-decompressor)
...
- `LoaderNetwork` registers all packets, discriminators `0`–`23` (full ID table in `.cursor/rules/network-packets.mdc` and `LoaderNetwork.java`).

When adding registrations, keep using centralized loader entry points.

## 5. Blocks, Items, and TileEntities

In-game display names below come from `assets/textech/lang/` (Chinese / English). Class header `Display names` comments match this table.
...
| `ItemManual` | 铽丝科技手册 | TeXTech Manual | `item.manual.name` |
...
Primary path: WebAE **Export for in-game display** publishes under `TeXTech/WebAE/displays/` and copies a binding. Successful monitor frames are only real web pixels: (1) client montoyo MCEF OSR when `webSurfaceUseMcef=true` (`mcef`), or (2) host Chrome/Edge headless capture of `/embed/dashboard?...&capture=1` (`spa-jpeg`). Compact `render.html` and AWT drawing-list snapshots are **not** successful live frames (`webSurfaceAllowLiveSnapshotFallback` defaults false; `render.html` is debug-only). Publish fills `webaeOrigin` from the browser Origin when possible. Never on the MC tick thread.

Client `WebSurfaceSourceRouter`: optional MCEF → CDP stub → HttpFrame; live mode does not silently fall back to Snapshot. GUI shows frame source + error codes; `GET /api/display/{id}/frame-status` exposes capture diagnostics.
...
| Action | Behavior |
|--------|----------|
| Right-click (server) | Toggles `droneEnabled` NBT — drone and projectile immunity |
| Sneak + right-click (client) | Opens `GuiSuperOrangeConfig` (`GuiHandler.SUPER_ORANGE_GUI_ID = 8`) |
| Settings GUI save | `PacketSuperOrangeConfig` (ID 22) → server writes nameplate, `matterBallEnabled`, `pickupMatterBallEnabled`, `dropMatterBallEnabled`, `dropMultiplier` |
...
`HandlerSuperOrange` handles projectile immunity, mining drop multiplier / matter-cluster merge, and pickup/toss matter-cluster merge (via `pickupMatterBallEnabled` / `dropMatterBallEnabled`).
...
Configuration is under `[superOrange]` (including `dropMultiplierMax` for GUI multiplier cap). See [Player Guide §3.10](../player/player-guide.md#310-super-orange).
...
2. `TeXTech.postInit()` calls `DataLoomCellHandler.register()`.
...
Registry name `manual` (lang: `item.manual.name` — TeXTech Manual).
...
**Package:** `com.imgood.textech.items.PlannerEntry`
...
**Package:** `com.imgood.textech.items.PlannerMergeMode`
...
**Package:** `com.imgood.textech.items.ItemAdvancePlanner`
...
Editing (editingSlotIndex >= 0, editingField != null)
  ...
  │   (empty confirm on new row: no-op; empty confirm on existing row: removeEntry, same as X)
...
**Package:** `com.imgood.textech.gui.guiscreen.GuiPlannerMergeConfirm`
...
public class LoaderItem {
    ...
    public static void registerItems() {
        ...
        advancePlanner = new ItemAdvancePlanner()
            ...
            .setTextureName("textech:advance_planner");
...
2. `LoaderItem.registerItems()`:
   ...
   - `setTextureName("textech:advance_planner")` → `textures/items/advance_planner.png`
...
##### 10.3 HUD Rendering (implemented)

Planner HUD is implemented in `renders/PlannerHudRenderer.java` and registered in `ClientProxy.init()` via `MinecraftForge.EVENT_BUS.register(new PlannerHudRenderer())`.

**Rendering behavior:**

- Listens to `RenderGameOverlayEvent.Post` with element type `TEXT` (not `ALL`).
- Scans the player inventory (hotbar first) for the first HUD-enabled Advance Planner (`ItemAdvancePlanner.isHudEnabled`); the item need not be held.
- Sorts with `PlannerMergeMode.BY_INDEX`, shows **incomplete entries only**, capped at `getHudMaxDisplay()`.
- Supports custom title, position ratios (`getHudPosX` / `getHudPosY`), scale (`getHudScale`), and text width (`getHudWidth`) from planner NBT; default title is `adm.planner.title`.
- Each line shows `#slotIndex` plus wrapped task text; empty text renders as `...`.

**Toggle:** Sneak + right-click the planner, or use the GUI (§9.5); HUD state and layout live in planner NBT (§6.3).

**Extending HUD:** Edit `PlannerHudRenderer` or reuse `HudRenderUtil` in the same package; new NBT fields require matching `ItemAdvancePlanner` accessors and lang keys.
...
- Chinese: `src/main/resources/assets/textech/lang/zh_CN.lang`
- English: `src/main/resources/assets/textech/lang/en_US.lang`
...
# HUD keys (defined in en_US.lang / zh_CN.lang as adm.planner.hud_*)
...
#### Fixed Issues (2026-06-30)

The following issues were fixed on 2026-06-30. Root cause and fix are recorded for reference.
...
### 5.13 Matter Ball Decompressor

> Player guide: [Player Guide §3.14](../player/player-guide.md#314-matter-ball-decompressor)

Key classes: `BlockMatterBallDecompressor`, `TileEntityMatterBallDecompressor`, `ContainerMatterBallDecompressor`, `GuiMatterBallDecompressor`, `MatterBallDecompressorUpgrades`, `MatterBallClusterUtil`, `AeUpgradeSpeedUtil`, `PacketMatterBallDecompressorToggle`, `RenderMatterBallDecompressor`.

**Architecture**

- Extends `AENetworkTile`, `GridFlags.REQUIRE_CHANNEL`, smart cable (`AECableType.SMART`).
- Left 9-slot `inputInv` accepts Avaritia matter clusters only; right 81-slot `bufferInv` (9×9 GUI).
- 4-slot `MatterBallDecompressorUpgrades`: `Upgrades.SPEED` / `Upgrades.SUPERSPEED` only (up to 4 each).
- Server tick: `decompressOneItem()` extracts one item from a cluster; when `outputToNetwork=true`, injects into `IStorageGrid`, otherwise or on overflow writes to buffer.
- **Block mode** (`blockMode`): only when `outputToNetwork=false`; pauses while buffer holds any item.
- Rate: `Config.matterBallDecompressorItemsPerSecond` (default 16/s) × `AeUpgradeSpeedUtil.getSpeedMultiplier()`.

**GUI / network**

- `GuiHandler.MATTER_BALL_DECOMPRESSOR_GUI_ID = 9`; mode toggles via `PacketMatterBallDecompressorToggle` (ID 23, `KIND_OUTPUT_MODE` / `KIND_BLOCK_MODE`).
...
> This document is based on TeXTech source. If code changes, prefer the actual source as authority.
...
- Resources: `assets/textech/config/assistant-lexicon.json` is fallback parser lexicon.
...
All debug switches live in `config/textech/textech.cfg`, **default false**:
...
When enabled, logs go through `TeXTech.LOG`.
